...
public class HoloCircularProgress extends View {
    ...
    public HoloCircularProgress(final Context context, final AttributeSet attrs,
        ...
        final TypedArray attributes = context
                .obtainStyledAttributes(attrs, R.styleable.HoloCircularProgress,
                        defStyle, 0);
        if (attributes != null) {
            try {
                setProgressColor(attributes
                        .getColor(R.styleable.HoloCircularProgress_progress_color, Color.CYAN));
                setProgressBackgroundColor(attributes
                        .getColor(R.styleable.HoloCircularProgress_progress_background_color,
                                Color.GREEN));
                setProgress(
                        attributes.getFloat(R.styleable.HoloCircularProgress_progress, 0.0f));
                setMarkerProgress(
                        attributes.getFloat(R.styleable.HoloCircularProgress_marker_progress,
                                0.0f));
                setWheelSize((int) attributes
                        .getDimension(R.styleable.HoloCircularProgress_stroke_width, 10));
                setThumbEnabled(attributes
                        .getBoolean(R.styleable.HoloCircularProgress_thumb_visible, true));
                setMarkerEnabled(attributes
                        .getBoolean(R.styleable.HoloCircularProgress_marker_visible, true));

                mGravity = attributes
                        .getInt(R.styleable.HoloCircularProgress_marker_gravity,
                                Gravity.CENTER);
            } finally {
